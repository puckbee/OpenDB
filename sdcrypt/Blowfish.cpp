...
void CBlowFish::Encrypt(unsigned char* buf, size_t n, int iMode)
{
	...
	if((n==0)||(n%8!=0)) { 
		printf("Incorrect buffer length\n");
	    return ; 
       }
	SBlock work;
...
void
CBlowFish::Char2Hex(const unsigned char ch, char* szHex)
{
...
void 
CBlowFish::Hex2Char(const char* szHex, unsigned char& rch)
{
	rch = 0;
	for(int i=0; i<2; i++)
	{
...
void 
CBlowFish::CharStr2HexStr(const unsigned char* pucCharStr, char* pszHexStr, int iSize)
{
	int i;
	char szHex[3];
	pszHexStr[0] = 0;
	for(i=0; i<iSize; i++)
	{
	  CBlowFish::Char2Hex(pucCharStr[i], szHex);
...
void 
CBlowFish::HexStr2CharStr(const char* pszHexStr, unsigned char* pucCharStr, int iSize)
{
	int i;
	unsigned char ch;
	for(i=0; i<iSize; i++)
	{
	  CBlowFish::Hex2Char(pszHexStr+2*i, ch);
	  pucCharStr[i] = ch;
	}
}
